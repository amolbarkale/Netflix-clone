...
import { Modal, Typography, Button, ButtonGroup, Grid, Box, CircularProgress, Rating, useMediaQuery, fabClasses } from '@mui/material';
import { Movie as MovieIcon, Theaters, Language, PlusOne, Favorite, FavoriteBorderOutlined, Remove, ArrowBack } from '@mui/icons-material';
import { Link, useParams } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import axios from 'axios';
import { useEffect, useState } from 'react';
import { useGetListQuery, useGetMovieQuery, useGetRecommendationQuery } from '../../services/TMDB.js';
  ...
  return (
    ...
    <Grid container className={classes.containerSpaceAround}>
          ...
          <div className={classes.buttonsContainer}>
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup size="small" variant="outlined">
                ...
                <Button onClick={() => setOpen(true)} href="#" endIcon={<Theaters />}>Trailor</Button>
              </ButtonGroup>
            </Grid>
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup size="medium" variant="outlined">
                ...
                <Button sx={{ borderColor: 'primary.main' }} endIcon={<ArrowBack />}>
                  <Typography color="inherit" component={Link} to="/" variant="subtitle2" style={{ textDecoration: 'none' }}>
                    Back
                  </Typography>
                </Button>
              </ButtonGroup>
      ...
      <Box marginTop="5rem" width="100%">
        ...
        {recommendations
          ? <MovieList numberOfMovies={12} movies={recommendations} /> : <Box>Nothing, was found</Box>}
      </Box>
      <Modal closeAfterTransition className={classes.modal} open={open} onClose={() => setOpen(false)}>
        {data?.videos?.results?.length > 0 && (
          <iframe autoPlay className={classes.videos} frameBorder="0" title="Trailer" src={`https://www.youtube.com/embed/${data.videos.results[0].key}`} allow="autoPlay" />
        )}
      </Modal>
    </Grid>
  );
